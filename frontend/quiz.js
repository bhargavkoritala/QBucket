...
function() 
 {
  ...
  function radioButtons(index) 
    {
        var radioItems = $('<ul style="list-style-type:none">');
        var item;
        var input = '';
        for (var i = 0; i < allQuestions[index].options.length; i++) {
          item = $('<li>');
          input = '<div class="form-check"><input class="form-check-input" type="radio" name="'+'answer'+'" id="exampleRadios1" value="'+i+'"><label class="form-check-label" for="exampleRadios1">'+allQuestions[index].options[i]+'</label></div>';
          //input = '<input type="radio" name="answer" value=' + i + ' />';
          console.log(allQuestions[index].options[i]);
